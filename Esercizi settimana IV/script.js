...
const marco = {
  ...
  for (let user of users) {
    ...
    if (totalCost > 100) {
      console.log(`Spedizione gratuita per ${user.name} ${user.lastName}`);
    } else {
      totalCost += shippingCost;
      console.log(`Costo spedizione: ${shippingCost} per ${user.name} ${user.lastName}`);
    }
  
    console.log(`Costo totale per ${user.name} ${user.lastName}: ${totalCost}`);
  }
